...
export function FinalReveal() {
  ...
  const { canvasRef, render, ready, count } = useFrameSequence("final", {
    fit: "contain",
    matte: MATTE,
    // Phones: larger watch, sitting in the upper part of the panel above the copy.
    mobileZoom: 1.45,
    mobileCenterY: 0.36,
  });
  ...
  return (
    <section ref={sectionRef} id="watch" className="relative bg-bg">
      <div className="mx-auto flex min-h-[100svh] max-w-[1400px] items-center px-4 pb-4 pt-20 md:px-10 md:pb-6 md:pt-24 short:pb-2 short:pt-[4.5rem]">
        <div
          ref={panelRef}
          className="relative h-[calc(100svh-6rem)] w-full overflow-hidden panel-start bg-panel text-panel-ink will-change-transform md:h-[calc(100svh-7.5rem)] short:h-[calc(100svh-5.25rem)]"
        >
          {/* Only the watch and wordmark are feathered; the copy below stays crisp. */}
          <div className="feather absolute inset-0">
            <canvas
              ref={canvasRef}
              role="img"
          ...
          <div aria-hidden className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-panel via-panel/80 to-transparent md:h-2/5" />

          <div className="absolute inset-x-0 bottom-0 grid grid-cols-1 items-end gap-5 p-5 md:grid-cols-12 md:gap-8 md:p-12 short:gap-4 short:p-5">
            <div className="md:col-span-5">
              <p data-reveal className="reveal-hidden font-mono text-xs uppercase tracking-[0.18em] text-panel-muted">
                Drop {drop.number}
              </p>
              <h1 data-reveal className="reveal-hidden mt-3 text-[2.75rem] leading-none tracking-[-0.03em] md:mt-4 md:text-7xl short:mt-2 short:text-4xl">
                {drop.model}
              </h1>
              <p data-reveal className="reveal-hidden mt-3 flex flex-wrap md:mt-5 items-baseline gap-x-4 gap-y-1 short:mt-2">
            ...
            <div data-reveal className="reveal-hidden flex flex-row items-end justify-between gap-4 short:gap-3 md:col-span-4 md:col-start-9 md:flex-col md:items-end md:justify-start md:gap-6">
              <div className="flex flex-col gap-2 short:gap-1.5 md:items-end md:gap-3">
                <span className="text-sm text-panel-muted">Closes in</span>
                <Countdown until={drop.closesAt} />
              </div>
              {/* TODO: point to checkout once it exists. */}
              <ButtonLink href="#checkout" variant="light" className="shrink-0 md:self-end">
                Reserve
              </ButtonLink>
            </div>
          </div>
